...
| `render/canvas-renderer.ts` | `drawRoom0Frame()` | Draws the recovered 40x25 screen through its 2 KiB charset and color RAM, then overlays VIC sprites at native coordinates. |
...
the captured coordinates/colors. The headless room runtime still advances the
verified `$F10E` state tables independently of drawing.

The page renders the screen, charset, and color RAM recovered by running `$633D`
against the side-1 frontend stream. An exact crop-normalized
emulator-to-browser pixel comparison is the next visual reverse-engineering
gate.
