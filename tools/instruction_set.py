#!/usr/bin/env python3
"""Documented MOS 6502/6510 opcode database.

Undocumented opcodes remain explicit one-byte ``???`` entries so disassembly
never silently loses synchronization. They will be classified as E1 analysis
finds actual uses.
"""
...
class Instruction:
    __slots__ = ("mnemonic", "mode", "size")

    def __init__(self, mnemonic: str, mode: str, size: int) -> None:
...
def _instruction(mnemonic: str, mode: str) -> Instruction:
    return Instruction(mnemonic, mode, MODE_SIZES[mode])


OFFICIAL: dict[int, tuple[str, str]] = {
    ...
    0xDE: ("DEC", "absx"), 0xE0: ("CPX", "imm"), 0xE1: ("SBC", "indx"),
...
# Undocumented opcodes are added only after a concrete target use is verified.
# The DOX depacker uses DCP zp at relocated address $00DA to decrement and
# compare the high byte of its self-modifying backwards source pointer.
OFFICIAL[0xC7] = ("DCP", "zp")


OPCODES: tuple[Instruction, ...] = tuple(
    _instruction(*OFFICIAL[opcode])
    if opcode in OFFICIAL
    else Instruction("???", "imp", 1)
