...
- **Verified (first visible web slice):** `web/index.html` connects the room-0 data and tick runtime to a native 320x200 Canvas 2D frame. The generated artifact decodes VIC pointers `$21/$22` from `$0840/$0880`; the renderer places them at captured coordinates `(64,88)/(64,109)` with colors `$9/$2`, while a separately labeled probe visualizes the confirmed slot-2 coordinate transform. Browser inspection found no console errors and observed the live tick advance. The corrupt/incomplete VICE background capture is not presented as a faithful Chatsubo room.
...
- [ ] Recover room-0 screen, character, and color RAM for a real Chatsubo background
