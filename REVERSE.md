...
- **Verified (VICE entity initialization):** the post-overlay scan at `$621B-$628B` activates room-0 record `$C400`: offset 5 changes `$FF->$00`, offsets 6-7 become script pointer `$F02D`, and only status slot 0 is selected. The other three records retain `$FF` at offset 5 and null script pointers. Dispatch then resolves `$C400/$F02D`, reads opcode `$01`, and selects handler `$64B5`; running that UI-bearing handler in isolation still blocks without the rest of the live frame state.
